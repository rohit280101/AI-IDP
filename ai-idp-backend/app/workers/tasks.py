...
def process_document(document_id: int):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Extracted {len(raw_text)} characters from document {document_id}")
            cleaned = clean_text(raw_text)
            document.raw_text = raw_text
            document.cleaned_text = cleaned
            db.commit()
            logger.info(f"Text saved to database for document {document_id}")
        ...
        try:
            if document.cleaned_text and document.cleaned_text.strip():
                ...
                logger.warning(f"Skipping embeddings for document {document_id} - no cleaned text")
            ...
            if document.cleaned_text and document.cleaned_text.strip():
                try:
                    ...
                    if classification:
                        ...
                        logger.info(f"Classification completed for document {document_id}: {classification}")
                    else:
                        logger.warning(f"Classification returned None for document {document_id}")
                        document.classification = None
                except Exception as e:
                    logger.error(f"Classification failed for document {document_id}: {e}", exc_info=True)
                    document.classification = None
            else:
                logger.warning(f"Skipping classification for document {document_id} - no cleaned text")
                document.classification = None
        except Exception as e:
            logger.error(f"Unexpected error during classification for document {document_id}: {e}", exc_info=True)
            document.classification = None
        ...
        if document:
            ...
            try:
                ...
                logger.error(f"Failed to commit error state for document {document_id}: {commit_err}")
